    ...
    def build(self,
        ...
        if self._num_decodes > 0:
            ...
            if use_torchair_graph and self.runner.attn_state == AscendAttentionState.DecodeOnly:
                ...
                block_table_padding = torch.zeros((graph_pad_size,) + block_table.shape[1:],
                       dtype=block_table.dtype,
                       device=block_table.device)
                block_table = torch.cat([block_table, block_table_padding], dim=0)
                block_table = self._get_graph_runner_block_tables(
                num_seqs, block_table)
                padding_0 = torch.zeros(graph_pad_size, dtype=input_positions.dtype, device=input_positions.device)
...
class AscendMLAImpl(MLAAttentionImpl):
    ...
    def __init__(
        ...
        self.rotary_emb_forward =  self.rotary_emb.forward_native
    ...
    def _v_up_proj_and_o_proj(self, x):
        ...
        if self.running_in_graph:
            # Convert from (B, N, S, D) to (N, B, L)
            #(4, s6, 512)
            #x = x.view(num_tokens, -1, self.kv_lora_rank).transpose(0, 1)
    ...
    def exec_kv(
        ...
        #ckv_cache, kpe_cache = kv_cache.split(
        #                [self.kv_lora_rank, self.qk_rope_head_dim], dim=-1)
        #torchair.ops.npu_print("ckv_cache shape", torch.tensor(ckv_cache.shape))
        #torchair.ops.npu_print("ckv_cache", ckv_cache)
        #torchair.ops.npu_print("kpe_cache shape", torch.tensor(kpe_cache.shape))
        #torchair.ops.npu_print("kpe_cache", kpe_cache)
        ...
        #torchair.ops.npu_print("k_pe", kpe_cache[0][6])
        #torchair.ops.npu_print("decode k_pe", k_pe[1], summarize_size = 8)
        #torchair.ops.npu_print("decode k_nope", k_nope[1], summarize_size = 8)
    ...
    def _forward_decode(
        ...
        if self.running_in_graph:
                ...
                #torchair.ops.npu_print("decode q_node", q_nope)
                #torchair.ops.npu_print("decode q_pe", q_pe)
                #torchair.ops.npu_print("actual_seq_lengths_kv", attn_metadata.decode.seq_lens)
                #torchair.ops.npu_print("block_table shape", torch.tensor(attn_metadata.decode.block_table.shape))
                #torchair.ops.npu_print("block_table", attn_metadata.decode.block_table)
    ...
    def forward(
        ...
        if has_decode:
            ...
            if self.running_in_graph:
                #num_tokens = hidden_states_or_kv_c_normed.shape[0]
                #q = self.q_proj(hidden_states_or_q_c)[0].view(-1, self.num_heads,
                #                                      self.qk_head_dim)
                #decode_ql_nope, decode_q_pe = q.split([self.qk_nope_head_dim, self.qk_rope_head_dim],
                #               dim=-1)
                seq_len = self.rotary_emb.max_position_embeddings
                cos = self.rotary_emb.cos_cached[:seq_len].to(dtype=decode_q_pe.dtype)
                sin = self.rotary_emb.sin_cached[:seq_len].to(dtype=decode_q_pe.dtype)
                ...
                decode_k_pe, decode_k_nope = self.exec_kv(hidden_states_or_kv_c_normed, cos, sin,
                                        kv_cache, attn_metadata.slot_mapping)

            else:
                decode_q_pe[...], decode_k_pe[...] = self.rotary_emb_forward(
                    attn_metadata.decode.input_positions, decode_q_pe.contiguous(),
                    decode_k_pe)
            #torchair.ops.npu_print("decode_k_pe shape", torch.tensor(decode_k_pe.shape))
            #torchair.ops.npu_print("decode_k_pe", decode_k_pe)
            #torchair.ops.npu_print("decode_q_pe shape", torch.tensor(decode_q_pe.shape))
            #torchair.ops.npu_print("cos", cos)
            #torchair.ops.npu_print("sin", sin)
            #torchair.ops.npu_print("decode_q_pe", decode_q_pe)
        if has_prefill:
            assert attn_metadata.prefill is not None
            ...
            if self.enable_graph_mode:
                ...
                if self.rotary_emb.__class__.__name__ == 'RotaryEmbedding':
                    ...
                    prefill_q_pe, prefill_k_pe = self.rotary_emb(attn_metadata.prefill.input_positions,
                    ...
                    prefill_q_pe, prefill_k_pe = self.rotary_emb(attn_metadata.prefill.input_positions,
                                             prefill_q_pe, prefill_k_pe)
                #torchair.ops.npu_print("prefill k_pe", prefill_k_pe, summarize_size = 8)
                #torchair.ops.npu_print("k_nope", k_nope, summarize_size = 8)
                prefill_q = torch.cat([prefill_q_nope, prefill_q_pe], dim=-1)
            else:
                prefill_q_pe[...], prefill_k_pe[...] = self.rotary_emb_forward(
            ...
            if len(kv_cache) > 0 and kv_cache[0].numel(
                ...
                torch_npu._npu_reshape_and_cache(key=kv_c_normed.view(
                    num_tokens, self.num_kv_heads, -1),
                                                 ...
                                                 slot_indices=slots)
                #torchair.ops.npu_print("cache k_pe", prefill_k_pe, summarize_size = 8)
                #torchair.ops.npu_print("cache slots", attn_metadata.slot_mapping)
                #torchair.ops.npu_print("cache kv_cache", kv_cache[1], summarize_size = 8)
                #slots = attn_metadata.slot_mapping
                # NOTE: Seperate the kv cache in advance to avoid OOM or other issues
                #torch_npu._npu_reshape_and_cache(key=k_c_normed.view(
                #    attn_metadata.num_actual_tokens, self.num_kv_heads, -1),
                #                                 value=k_pe,
                #                                 key_cache=kv_cache[0],
                #                                 value_cache=kv_cache[1],
                #                                 slot_indices=slots)
        ...
        if has_decode:
            if self.running_in_graph:
                return self._forward_decode(
                    decode_ql_nope, decode_q_pe, decode_k_nope, decode_k_pe, kv_cache, attn_metadata)
            else:
